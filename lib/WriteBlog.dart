...
class WriteBlog extends StatefulWidget {
  ...
  final String uid;
  WriteBlog({Key? key, required this.isEditable, required this.uid}) : super(key: key);
  @override
  _WriteBlogState createState() => _WriteBlogState(uid, isEditable);
}

class _WriteBlogState extends State<WriteBlog> {
  var readUid, isEditable;
  _WriteBlogState(this.readUid, this.isEditable);
  String result = '', headline='', imgUrl='', textOnlyResult='', uid='';
  late int i = 0, wordCount = 0, linksCount = 0, ytVideosCount = 0, ImagesCount = 0;
              ...
              Stack(
                children: [
                  Container(
                    height: 100,
                    width: 100,
                    child: imgUrl == '' ? Image.asset('assets/noimage.jpg'):Image.network(imgUrl),
                  ),
                  Positioned(
                    top: 0,
                    right: 0,
                    child: InkWell(
                      onTap: () async{
                        uid == '' ? {doc = firestoreInstance.collection("Drafts").doc(),
                        uid = doc.id,}: {};
                        uploadToStorage();
                      },
                      child: Container(
              ...
              Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: headline == '' ? 'Write your title' : headline,
  ...
  void saveBlog(String collection) {
    try {
      ...
      firestoreInstance.collection(collection).doc(uid).set({
        ...
        "tags": collection == "Blogs" ? _controller.hasTags ? _controller
            .getTags : [] : [],
      });
      collection == "Blogs" ? firestoreInstance.collection("Drafts")
          .doc(uid)
          .delete() : {};
      collection == "Trash" ? firestoreInstance.collection("Drafts")
          .doc(uid)
          .delete() : {};
    }catch(error){
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Some Issue Occurred')));
    }
  }
  ...
  getBlogInformation(){
    final res = FirebaseFirestore.instance.collection("Blogs").doc(readUid).get();
    res.then((value){
